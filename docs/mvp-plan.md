...
### Phase 10 — Deploy — **In progress** (started 2026-09-22; runbook in [deploy.md](deploy.md))

- [x] 1. Vercel CLI as a root dev dependency (`pnpm vercel`), `.vercel/` gitignored; owner logged in and linked two projects with a repo link: `discount-maps-people` (`apps/people-web`) and `discount-maps-business-web` (`apps/business-web`), Root Directory set, Node 24
- [x] 2. `scripts/vercel-env-sync.mjs` pushed every `.env.example` variable from `.env.local` into both projects for production + preview (URL variables overridden with the `*.vercel.app` origins, non-public names stored as sensitive, values never printed); first deploy uses the Wompi **sandbox** keys and the dev VAPID pair by decision
- [x] 3. Production deploys from the CLI (`vercel deploy --prod` per app, builds ~35 s): `https://discount-maps-people.vercel.app` and `https://discount-maps-business-web.vercel.app` serve the landings, manifests, service workers, icons and offline pages; gated routes redirect to `/login`; signing in as `qa-consumer@` on production lists the two Pizzas del Norte rows on `/mapas` and registers the service worker. Still to do: connect the GitHub repo to both projects for auto-deploys (dashboard → Settings → Git)
- [ ] 4. Owner: Vault secrets (`people_web_url`, `billing_cron_secret`, `push_dispatch_secret`), Supabase Site URL + redirect URLs, Wompi events URL, Maps key referrers (exact values in deploy.md § 3–4); then verify the hourly cron hits `/api/billing/run` and a real phone gets a push
- [ ] 5. Later: custom domains, production Wompi keys, production VAPID keys, delete QA fixtures, leaked-password protection
...
| 2026-09-22 | Deploy on Vercel as two projects (`discount-maps-people`, `discount-maps-business-web`) linked at the repo level; env vars are synced from the owner-pasted `.env.local` files by `scripts/vercel-env-sync.mjs` (values piped to the CLI, never printed, non-public names sensitive). First production deploy keeps the Wompi **sandbox** keys and the dev VAPID pair. | Owner decision: a safe smoke deploy first, real keys swapped in the dashboard later; the script keeps the "agents never handle secrets" rule while still automating the setup. |
